...
prec=[0.10,0.50,0.60,0.64]
cols=["#c0392b","#e08e0b","#2e86c1","#2e86c1"]
fig,ax=plt.subplots(figsize=(7,4.6))
bars=ax.bar(range(4),prec,color=cols,edgecolor="white",width=0.55)
ax.axhspan(0.60,0.64,color="#5dade2",alpha=0.18)
ax.text(1.5,0.665,"dictionary polysemy ceiling \u2248 0.60\u20130.64",ha="center",fontsize=8.5,color="#1a5276")
ax.axhline(0.75,ls=":",color="#888",lw=1.2); ax.text(3.35,0.765,"pre-registered 0.75 bar",ha="right",fontsize=8,color="#666")
...
ax.set_ylabel("visible-class precision (vs human gold, n=120)")
ax.set_ylim(0,0.98)
ax.set_title("Lexicon precision ladder: curation hits a polysemy ceiling;\nan LLM ensemble reaches human-level agreement",fontsize=10.5)
...
fig.text(0.5, 0.02, "LLM ensemble (Gemini 3.1 Pro + ChatGPT 5.5 + DeepSeek): agrees with human coders at \u03ba = 0.66\n(on par with human\u2013human \u03ba = 0.70); the concrete dictionary matches the ensemble majority on 84% of n = 499 sentences", ha="center", fontsize=8.5, color="#1a5276")
