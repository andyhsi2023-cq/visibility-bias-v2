# Cover Letter — Journal of Computational Social Science (Revised Submission)

**Date**: 2026-06-19
**Manuscript**: JCSO-D-26-00240 (Major Revisions)
**To**: Editor-in-Chief, *Journal of Computational Social Science* (Springer Nature)
...
**What the revision does.** The review correctly judged that our original draft measured visible-infrastructure *rhetoric* more convincingly than strategic *visibility bias*, and overclaimed. We have **kept the Visibility Attention Index (VAI) as the paper's instrument and kept its structure**, and used your comments to **strengthen the validation and narrow the claims** — the paper a reviewer reads is the same one, much better evidenced. Applied to 6,294 Chinese municipal government work reports (282 prefectures, 2002–2024), the contribution remains methodological and, we believe, of direct interest to JCSS's readership:

1. **A rigorous passage-level validation that reports a precision ceiling.** Against human double-coding, a naive visibility dictionary fails (visible-class precision 0.10, from polysemous appearance words); a concrete, salience-based lexicon improves greatly (0.50→0.60) but meets a **polysemy ceiling (~0.60–0.64)** that curation cannot break. Human intercoder agreement is Cohen κ = 0.70.
2. **A calibrated high-precision alternative.** A three-model **LLM ensemble** attains 0.84 accuracy (inter-model Fleiss κ = 0.835), benchmarked against the same human gold standard — a concrete precision-vs-transparency trade-off.
...
**Reproducibility.** Pre-registration: OSF DOI [10.17605/OSF.IO/ZMJY5](https://doi.org/10.17605/OSF.IO/ZMJY5). Replication archive (instrument, gold-standard codings, all analysis and re-pull code, procurement classification, deviation log): Zenodo concept DOI [10.5281/zenodo.19569978](https://doi.org/10.5281/zenodo.19569978) (always resolving to the latest version), mirrored at GitHub (`andyhsi2023-cq/visibility-bias-v2`). We have re-deposited and verified that every link resolves: the Zenodo record is now **open access** and the GitHub mirror is **live and public**. We apologize that the original record was not accessible to the reviewer and have corrected it.

**Use of generative AI.** Large-language-model classification is an *object of study and a measurement tool* in this paper (the §3.2.0 ensemble), documented and reproducible; separately, the authors used an LLM for copy-editing only. All substantive content and conclusions are the authors' own.

We suggest, as before, Daniel Mattingly (Yale), Jennifer Pan (Stanford), and Yuhua Wang (Harvard) as reviewers, and note any recusals in `suggested_reviewers.md`. Thank you for the opportunity to revise.
