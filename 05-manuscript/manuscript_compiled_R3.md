...
We introduce the Visibility Attention Index (VAI), a pre-registered text instrument measuring the share of *visible* versus *functional* infrastructure language in bureaucratic documents, applied to 6,294 Chinese municipal government work reports (282 prefectures, 2002–2024). VAI is motivated by a cadre-attention model. We separate what it measures, visible-versus-functional language, from the stronger claim of strategic visibility bias. We validate VAI at the passage level against human coding (two coders, Cohen κ = 0.70) and report its limits plainly. A naive appearance-word dictionary reaches only 0.10 precision, because polysemous image terms (形象 *xíngxiàng* "image"; 示范 *shìfàn* "model") fire outside infrastructure. A concrete, salience-based lexicon reaches 0.50–0.60 but meets a polysemy ceiling, above which an LLM ensemble agrees with human coders on par with the coders' own agreement (κ = 0.66 vs 0.70), and the dictionary matches the ensemble majority on 84% of sentences (inter-model Fleiss κ = 0.835). We then add a behavioral criterion-validity test. Under quasi-exogenous, retirement-driven secretary turnover, the concrete VAI co-moves with real cosmetic investment (+0.010, p = 0.01; clean pre-trends) while the naive measure does not (+0.002); the valid measure tracks real resource allocation, not genre. Descriptively, within-city VAI tracks the Cosmetic Investment Ratio (β = +0.095), and an independent procurement corpus (≈2.96M Zhejiang records) corroborates the tilt, with visible projects running about 2-to-1 over functional in 97 of 108 localities. A failed third-party Wikipedia test (r = −0.15), an inspection event study, a CFPS micro-foundation, and a welfare calibration are reported transparently and demoted. The contribution is methodological: a pre-registered, passage-validated, behaviorally-anchored instrument, with an honest map of where the dictionary stops and an LLM alternative begins.
...
- **Text-as-data in political methodology.** A large literature measures political *attention* and *expressed agendas* from text, including expressed priorities in legislative communication, agenda-setting, audience-targeting, and cross-domain priority classification [2, 3, 4, 8, 16, 17]. VAI is a domain-specific instance: an interpretable, expert-coded measure of *compositional* (visible-vs-functional) attention in Chinese municipal reports.
...
The substantive premise is an observability asymmetry. The evaluating audience perceives visible categories with less noise than functional ones ($\omega_V^2 < \omega_F^2$). Three independent proxies for the asymmetry, namely inspection frequency, media-reporting intensity, and the visibility of failures in the accident record, are consistent with it (Online Appendix B.4). The conceptual move that matters here is that "visible to whom" is answered by the *upward* principal; lateral citizen perception is a distinct channel, which our individual-level analysis treats as a bounded null rather than the mechanism (§4.5).
...
Before describing the VAI construction, we situate the instrument relative to existing text-based measures of political attention. Table 0 compares VAI across five dimensions against three established instruments.
...
We draw a stratified random sample of sentences (by dictionary hit-type, city tier, era) and have them independently coded as visible, functional, mixed, or irrelevant. A human gold standard of 120 sentences, independently double-coded by two coders (dictionary and model labels hidden), anchors the analysis (intercoder Cohen κ = 0.70, substantial; Online Appendix B.8). Against the human labels we report a full confusion matrix, per-class precision/recall, verbatim error exemplars, and an error taxonomy. The findings define a precision ladder (Figure 1):
...
- **An LLM ensemble clears the ceiling.** Three independent model families (Gemini 3.1 Pro, ChatGPT 5.5, DeepSeek), each given the same four-class definition with the majority label taken, agree with the human gold standard at Cohen κ = 0.66 — on par with the two human coders' own agreement (κ = 0.70) — with inter-model Fleiss κ = 0.835. The recommended concrete dictionary in turn matches the ensemble majority on 84.2% of a 499-sentence ensemble-coded sample (Online Appendix B.8), consistent with the dictionary capturing nearly all of what the ensemble labels. This is the recommended route where high passage precision is required, consistent with recent evidence that LLMs match or surpass human and crowd annotation of political text [5, 18, 19] (the transparency cost is discussed in §5).
...
A second concrete lexicon, curated independently from a separate reading of 150 GWRs, yields VAI correlated with the primary at r = 0.93. This check was performed on the earlier lexicon versions and is reported as a comparison. We report it as internal reliability rather than independent construct validity: both lexicons are drawn from the same governance genre, so overlap is expected.
...
VAI correlates with the MOHURD yearbook-derived Cosmetic Investment Ratio (CIR) at r = 0.24 (p < 10⁻³⁰) over the 2005–2015 overlap (earlier lexicon version; comparison). This is independent evidence from accounting rather than text, modest in magnitude.
...
|---|---|---:|---|
| E-A independent-lexicon (earlier versions) | r(VAI, VAI_ext) | **0.93** | reliability, not validity |
...
| E-D yearbook cross-source (earlier version) | r(VAI, CIR) | **0.24** (p < 10⁻³⁰) | modest external anchor |
...
The pattern is exactly what criterion validity requires. In the year after turnover, real cosmetic investment rises and the concrete VAI rises in lockstep, both with clean pre-trends (the future-turnover placebo is null), while the naive VAI does not respond at all. We test the concrete–naive difference directly: on common observations and a comparable (standardized) scale, the concrete measure's turnover response is significantly larger than the naive measure's (difference in standardized coefficients +0.058, SE 0.029, Wald χ²(1) = 4.01, p = 0.045; the retirement-driven specification gives +0.081, p = 0.12, directionally the same). This behavioral co-movement, rather than passage precision, is the strongest evidence that the construct is real and that the *valid* (concrete) measure captures it. We read it as criterion validity for the measure rather than as a demonstration of strategic behavior: a new leadership team could also change real construction priorities, and the report would then accurately describe that change. The narrower descriptive interpretation — a measure of visible-infrastructure rhetoric that co-moves with real cosmetic investment under incentive shocks — is the paper's base claim.
...
with city ($\mu_i$) and year ($\tau_t$) fixed effects, controls $X_{it}=\{\ln\text{GDPpc}, \ln\text{pop}, \text{ind2-share}\}$, and city-clustered SE; sample 1,834 city-years across 281 cities, 2009–2015 (the MOHURD CIR coverage runs 2005–2015, but the population control is unavailable before 2009 in the merged panel).

**Table 3. P1 — Compositional substitution (CIR ~ VAI)**

| Specification | β(VAI) | SE | p | N |
|---|---:|---:|---:|---:|
| (1) Recommended VAI (`wr_visibility`) | **+0.095** | 0.042 | **0.025** | 1,834 |
| (2) Naive VAI (`vai_composite`) | +0.063 | 0.067 | 0.349 | 1,834 |
| (3) No city FE | +0.069 | 0.044 | 0.119 | 1,834 |
| (4) No year FE | +0.108 | 0.042 | 0.011 | 1,834 |
| (5) IV = VAI(t−1) | +0.313 | 0.424 | 0.460 | 1,826 |
| (6) Null: total investment | +0.220 | 0.186 | 0.237 | 1,834 |

The recommended measure is positive and significant in the primary specification and without year effects; the naive measure, on the same sample, is not significant (row 2). Dropping city effects (row 3) weakens the association below the 5% bar, and the lagged-VAI instrument (row 5) is imprecise (first-stage F = 17.2 but a wide confidence interval), so we do not lean on either. The association with total investment (row 6) is not significant, consistent with *composition, not expansion*. Horse races against tertiary-sector share, leader tenure, GDP growth, and cumulative inspection pressure leave β(VAI) essentially unchanged, significant in every specification (Online Appendix B.2). We read this as a descriptive compositional-substitution association, conditional on the instrument's validity, and stop short of a causal claim.

## 4.2 Application II: The within-document review-vs-plan differential (suggestive)

Exploiting the temporal structure of GWRs (a retrospective past-year review and a prospective next-year plan), we partition each report at the first plan marker after character 2000 and compute VAI on each section (N = 4,330 successful splits). This exercise uses the earlier lexicon version and is reported as a comparison; it is not part of the recommended instrument's evidence (Appendix B.1).

**Table 4. Within-document retrospective-vs-prospective VAI differential (earlier lexicon version; comparison)**
...
The differential is compatible with the upward-signaling mechanism but is not a unique diagnostic. Downstream-facing rhetoric, lexical-availability bias, and selection on what gets completed-versus-planned would also predict it. Robustness to alternative split markers and to long-document/single-tenure subsamples is in Online Appendix B.3.
...
Two pre-registered tests are demoted from the main evidentiary structure (full detail in Appendix B.3 and Appendix D):

**Null 1, central-inspection event study (H2): definitively null.** A narrow-window TWFE specification gave the predicted sign (β(k=0) = −0.065, p = 0.011), but a heterogeneity-robust Sun–Abraham estimator attenuates it to insignificance and, extending to Rounds 6–9 (2016–2017), reverses the sign (β = +0.016). The narrow-window result is a staggered-treatment-heterogeneity artifact [13, 14, 15] rather than a causal inspection effect (deviations D-B-1/2/3).
...
- Direct passage-level validation (§3.2.0): the naive appearance-word dictionary fails (visible precision 0.10); the concrete, salience-based VAI reaches 0.50–0.60 but meets a polysemy ceiling near 0.60–0.64; an LLM ensemble attains human-level agreement with the gold standard (κ = 0.66 vs human–human κ = 0.70; inter-model Fleiss κ = 0.835).
...
- A cross-source accounting correlation (E-D, r = 0.24; earlier lexicon version) provides independent external evidence, modest in magnitude. An independent-dictionary replication (E-A, r = 0.93) is a reliability check rather than a validation. And an independent procurement corroboration (§4.3) shows a ≈2:1 visible-over-functional tilt in 97 of 108 Zhejiang localities, with a visible-dominated spending tail.
...
Within cities, VAI tracks the accounting-based Cosmetic Investment Ratio (P1, β = +0.095, p = 0.02) with no significant accompanying change in total investment. The association is therefore compositional rather than expansionary, and under the turnover shock both rise together. We read these as evidence that the measure captures strategic compositional attention. We do not offer a headline welfare figure, because the model-based calibration (§4.4) is too assumption-dependent to support one.
...
We set out to measure visibility bias in Chinese municipal government work reports and to take seriously the question applied dictionary work usually skips: does the measure capture the construct? The Visibility Attention Index, built from 6,294 GWRs, is validated directly at the passage level. We report that validation honestly, including the naive dictionary's failure (precision 0.10), the concrete lexicon's polysemy ceiling (~0.60), and an LLM ensemble that attains human-level agreement (κ = 0.66). The behavioral test settles the question: under an exogenous retirement-turnover shock the valid measure co-moves with real cosmetic investment while the naive one does not (presented as criterion validity; the strategic interpretation is plausible but not established). Descriptively, VAI tracks the accounting Cosmetic Investment Ratio and is corroborated by an independent procurement corpus, where visible projects run ≈2:1 over functional and own the high-value tail. One pre-registered third-party test failed and is reported plainly. An inspection event study, a CFPS micro-foundation, and a welfare calibration are demoted as non-robust or assumption-dependent.
...
**Use of Generative AI.** Large-language-model classification is an *object of study and a measurement tool* in this paper (the three-family ensemble of §3.2.0), documented and reproducible in the replication archive. The authors additionally used LLM tools at several further stages, all under author supervision and verification: (i) an LLM-assisted adjudication step in refining the procurement classification lexicon (§4.3), reviewed and signed off by the domain-expert co-author; (ii) assistance with drafting and revising manuscript text, including copy-editing; and (iii) assistance with analysis and replication code. All substantive decisions, statistical results, interpretations, and the final text were produced, checked, and approved by the authors, who take full responsibility for the content. No LLM is an author.
...
- **H2 (P2)**: Staggered central-inspection arrival reduces VAI at k ≥ 0 by |Δa| ≥ 0.02. Estimator: Callaway and Sant'Anna [12] doubly-robust staggered DiD.
...
**Pre-registered**: Callaway and Sant'Anna [12] doubly-robust DiD with never-treated controls.
...
**Executed**: Sun and Abraham [13] cohort-stacked heterogeneity-robust estimator using 5 round-level inspection cohorts (Rounds 1–5, start dates 2013-05 through 2014-11). Cohort c uses later-treated cohorts' pre-treatment observations as controls.
...
**Interpretation**: All nine cohort-specific CATT(k=0) values under the expanded sample are positive (+0.011 to +0.083). The original TWFE negative coefficient was a staggered-treatment-bias artifact [14, 15], not a causal inspection effect. We report H2 as **definitively null** in §4.5. The pre-registered "consistent with prediction" framing from earlier drafts is withdrawn. We log this deviation honestly; it represents a substantive failed identification, not a methodological quibble.
...
- E-A: Expanded internal dictionary (78+70 terms, independently curated), giving r(VAI_orig, VAI_ext) = 0.93 (earlier lexicon versions; comparison).
...
- E-A CIR replication with VAI_ext, a further check on the earlier lexicon versions, gave β = +0.111, p = 0.011.
...
## Appendix B: Further Robustness and Sensitivity

### B.1 Dictionary details

**Recommended measure (carried through validation and applications).** The concrete VAI (`wr_visibility`) counts 20 visible terms — 绿化, 园林, 景观, 亮化, 公园, 绿地, 美化, 广场, 风貌, 绿道, 街景, 花园, 道路, 路网, 桥梁, 立交, 轨道, 地铁, 大道, 景观带 — against 16 concealed-utility terms — 排水, 排涝, 管网, 管道, 防洪, 地下管, 污水, 供水, 管廊, 内涝, 下水道, 排污, 给排水, 防汛, 地下空间, 雨水 — verbatim from `03-analysis/phase-J-criterion-validity/build_workreport_text.py` (the lists mirror the MOHURD cosmetic/functional construction-investment split). **Comparison versions (earlier lexicons).** V_ORIG (42 terms), F_ORIG (38 terms), V_EXT (+36 new terms, 78 total), F_EXT (+32 new terms, 70 total) are in `03-analysis/phase-E/construct_validity.py`; these underpin the E-A/E-B/E-D checks reported as comparisons. Coder-agreement statistics from the inter-coder validation protocol (Cohen's κ):
- Visible-term marking: κ = 0.87
- Functional-term marking: κ = 0.84
- Overall orientation (1–5 Likert): ICC = 0.79

### B.2 Horse races against alternative explanations (main text §4.1)

| Alternative | Specification | β(VAI) | p |
|---|---|---:|---:|
| Baseline | City FE + year FE + 3 controls | +0.095 | 0.025 |
| + tertiary-sector share | Added tertiary-sector share | +0.093 | 0.026 |
| + leader tenure | Added party-secretary tenure + age | +0.086 | 0.040 |
| + GDP growth | Added annual GDP growth | +0.093 | 0.026 |
| + inspection pressure | Added cumulative inspection rounds | +0.094 | 0.027 |
| All jointly | All above | +0.084 | 0.041 |

### B.3 P2 placebo battery (main text §4.5; earlier lexicon version)

**Table B.3.** Placebo tests for the inspection event study.
...
### B.4 Evidence for observability asymmetry (A1)
...
### B.5 Tenure-cycle dynamics (supplementary evidence; earlier lexicon version)
...
### B.6 Heterogeneity by fiscal capacity
...
|---|---:|---:|---:|
| Above-median fiscal capacity | +0.023 | 0.673 | 917 |
| Below-median fiscal capacity | +0.174 | 0.005 | 917 |
| Interaction (above × VAI) | −0.139 | 0.055 | 1,834 |

Contrary to the model's fiscal-capacity prediction (Online Appendix A.2), the association is concentrated in *below*-median fiscal-capacity cities; the interaction is negative (−0.139, p = 0.055). We report this descriptive reversal plainly rather than suppressing it.

### B.7 Welfare sensitivity (main text §4.4; earlier lexicon version; illustrative)
...
### B.8 Passage-validation confusion matrix and error taxonomy (main text §3.2.0)
...
**Table B.8a. Concrete (salience-based) dictionary vs human coding, n = 120.** Rows = human label, columns = dictionary prediction.
...
Per class: visible P = 0.500, R = 0.789, F1 = 0.61; functional P = 0.600, R = 0.900; mixed P = 0.800, R = 0.600; irrelevant P = 0.900, R = 0.659. Overall four-class accuracy = 0.70 (84/120).

**Table B.8b. Visible-class false-positive taxonomy (naive dictionary).** Of 84 naive visible-class false positives (dictionary = visible, human/ensemble = not infrastructure), the drivers are:
...
**Table B.8c. Concrete dictionary ↔ LLM-ensemble majority, n = 499.** Rows = three-model majority label; columns = dictionary prediction (same convention as Table B.8a). Overall agreement 0.842; inter-model Fleiss κ = 0.835. On the 120-sentence human gold standard, human↔ensemble agreement is Cohen κ = 0.66 (human–human κ = 0.70, §3.2.0). Source: `passage_coding_sheet_ensemble.validation.json`.
...
5. Ornstein, J. T., Blasingame, E. N., & Truscott, J. S. (2025). How to train your stochastic parrot: Large language models for political texts. *Political Science Research and Methods, 13*(2), 264-281. https://doi.org/10.1017/psrm.2024.64
...
13. Sun, L., & Abraham, S. (2021). Estimating dynamic treatment effects in event studies with heterogeneous treatment effects. *Journal of Econometrics, 225*(2), 175-199. https://doi.org/10.1016/j.jeconom.2020.09.006

14. Goodman-Bacon, A. (2021). Difference-in-differences with variation in treatment timing. *Journal of Econometrics, 225*(2), 254-277. https://doi.org/10.1016/j.jeconom.2021.03.014

15. Borusyak, K., Jaravel, X., & Spiess, J. (2024). Revisiting event-study designs: Robust and efficient estimation. *Review of Economic Studies, 91*(6), 3253-3285. https://doi.org/10.1093/restud/rdae007

16. Grimmer, J., & Stewart, B. M. (2013). Text as data: The promise and pitfalls of automatic content analysis methods for political texts. *Political Analysis, 21*(3), 267-297. https://doi.org/10.1093/pan/mps028

17. Osnabrügge, M., Ash, E., & Morelli, M. (2023). Cross-domain topic classification for political texts. *Political Analysis, 31*(1), 59-80. https://doi.org/10.1017/pan.2021.37
...
19. Heseltine, M., & Clemm von Hohenberg, B. (2024). Large language models as a substitute for human experts in annotating political text. *Research & Politics, 11*(1). https://doi.org/10.1177/20531680241236239
