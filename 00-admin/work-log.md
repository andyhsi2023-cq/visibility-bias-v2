...
> ⚠️ **2026-09-11 corrections**: see `00-admin/CORRECTION-2026-09-11.md` — (1) the 2026-06-19 entry naming 李志辉 as the second passage coder was a naming error; the second coding was performed by Liu Can. (2) Some early public archive versions unintentionally included confidential correspondence; remediation described in the correction note.
...
## 2026-09-11 — 第二轮 Major Revisions 决定到达（R2）

- **决定邮件**:2026-09-09 12:57 收到 EM 邮件「Major Revisions requested JCSO-D-26-00240R1」；主编 Takashi Kamihigashi；**截止 2026-10-22**；提交须可编辑源文件（Word/Tex，禁 PDF）。
- **审稿意见**:仅 Reviewer #1，附件 PDF 已下载 → `06-submission/JCSO_R1_comments_to_authors.pdf`（EM Attachments 页，Revision 1 / Reviewer 1）。**无第二轮新增审稿人**。
- **基调**:认可 R1 进展（passage coding / 复现材料 / inspection-CFPS-welfare 降级），仍认为有潜力的 measurement paper，但要求**进一步 Major Revision**；明言这些关切「不构成故意失实」。
- **四大类意见**（详见 response 骨架）：
  1. 测量解释：换届分析不解决修辞 vs 战略偏好；Table 2 concrete-vs-naive 需同观测直接差异检验；晋升考核断言需证据或走描述路线。
  2. 验证一致性：Table C.8c 的 0.84 声称（vs 人工）与存档记录（词典 vs LLM 多数票）不符；Table C.8a accuracy 与混淆矩阵不一致；**验证的测度 ≠ 应用的测度**（Table 3 存档为旧版测度、§4 按新工具呈现、构建代码与 §3.1.1 规则不同）。
  3. 可复现性：Table 3 行 4–6 + specification curve 代码/输入不完整；OSF 注册不可访问；turnover 分析与 passage-coding 修订未在原始注册中、须区分 prespecified / amendment / exploratory。
  4. 引用与声明：**Ref 12（Table 0 所用）DOI 不解析、标题搜不到**；Ref 5 DOI 错误；AI 声明与存档记录（实质性重写+分析工作）不一致；第二编码归因（work-log 记李志辉 vs 提交贡献声明记刘灿）矛盾；**公开存档含早期审稿报告/投稿通信/外联邮件/审稿人偏好与联系方式** → 需与编辑部讨论授权发布并脱敏。
- **下一步**:response-to-reviewers-R2 骨架 + 逐条核对（引用/存档/测度一致性）→ Andy 确认后改稿。
